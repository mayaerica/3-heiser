...
func SendMessage(msg Message) {
	...
	for id, addr := range elevatorAddresses {
		if id != fsm.Elevator.Id { // Avoid sending to itself
			...
			// Send the message to the connected elevator
			defer conn.Close()
			...
			if err != nil {
				...
				fmt.Printf("Message sent to Elevator %d: %+v\n", id, msg)
			}
		}
	}
}
